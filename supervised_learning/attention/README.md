...
Contains the class `EncoderBlock` that inherits from
`tensorflow.keras.layers.Layer` to create an encoder block for a
transformer.
...
- Uses `MultiHeadAttention` from `6-multihead_attention.py`
- Returns: a tensor of shape `(batch, input_seq_len, dm)` containing
  the block's output

### 8-transformer_decoder_block.py
Contains the class `DecoderBlock` that inherits from
`tensorflow.keras.layers.Layer` to create a decoder block for a
transformer.
...
Contains the class `Encoder` that inherits from
`tensorflow.keras.layers.Layer` to create the encoder for a
transformer.
...
- `training` - a boolean to determine if the model is training
- `mask` - the mask to be applied for multi head attention
- Uses `positional_encoding` from `4-positional_encoding.py` and
  `EncoderBlock` from `7-transformer_encoder_block.py`
- Returns: a tensor of shape `(batch, input_seq_len, dm)` containing
  the encoder output

### 10-transformer_decoder.py
Contains the class `Decoder` that inherits from
`tensorflow.keras.layers.Layer` to create the decoder for a
transformer.
...
- `padding_mask` - the mask to be applied to the second multi head
  attention layer
- Uses `positional_encoding` from `4-positional_encoding.py` and
  `DecoderBlock` from `8-transformer_decoder_block.py`
- Returns: a tensor of shape `(batch, target_seq_len, dm)` containing
  the decoder output

### 11-transformer.py
Contains the class `Transformer` that inherits from
...
- Uses `Encoder` from `9-transformer_encoder.py` and `Decoder` from
  `10-transformer_decoder.py`
