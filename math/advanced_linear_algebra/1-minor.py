#!/usr/bin/env python3
"""Matrisin minorlarını hesablayan modul"""
...
def minor(matrix):
    ...
    for i in range(n):
        row_minors = []
        for j in range(n):
            # i-ci sətri və j-cu sütunu silərək sub-matrisi yaradırıq
            sub_matrix = [row[:j] + row[j+1:] for row in
                          (matrix[:i] + matrix[i+1:])]
            # Həmin sub-matrisin determinantı bizim minorumuzdur
